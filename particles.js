document.addEventListener("DOMContentLoaded", () => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
        ...
        p.style.cssText = `
            ...
            opacity: 0;
            filter: ${isGlow
                ? `blur(2px) drop-shadow(0 0 ${size * 3}px rgba(255,59,59,0.9))`
                : isBright
                    ? `drop-shadow(0 0 ${size}px rgba(255,59,59,0.7))`
                    : 'none'
            };
            background: ${isBright
                ? 'radial-gradient(circle, #ff6b6b 0%, #ff3b3b 60%, transparent 100%)'
                : '#ff3b3b'
            };
        `;
    ...
    function draw() {
        if (!doomRunning) { // No dibujar si Doom está activo
            ...
            for (const node of NODES) {
                ...
                grd.addColorStop(0, `rgba(255,80,80,${pulseAlpha})`);
            ...
            for (let i = 0; i < NODES.length; i++) {
                for (let j = i + 1; j < NODES.length; j++) {
                    ...
                    if (dist < MAX_CONN_DIST) {
                        const alpha = (1 - dist / MAX_CONN_DIST) * 0.18;
                        const grd2 = ctx.createLinearGradient(a.x, a.y, b.x, b.y);
                        grd2.addColorStop(0, `rgba(255,59,59,${alpha})`);
                        grd2.addColorStop(0.5, `rgba(255,30,30,${alpha * 1.5})`);
                        grd2.addColorStop(1, `rgba(255,59,59,${alpha})`);
            ...
            for (let i = waves.length - 1; i >= 0; i--) {
                ...
                if (wave.r > 20) {
                    ctx.strokeStyle = `rgba(255,80,80,${a * 0.4})`;
                    ctx.lineWidth = 0.5;
                    ctx.beginPath();
            ...
            vignette.addColorStop(1, 'rgba(80,0,0,0.22)');
