...
//
//  Transporte independiente del chat normal:
//   - Usa api/workspace-model-client.js exclusivamente con OpenRouter
//   - Nunca importa ni llama a api/groq-client.js
...
// Solo para el Sandbox. El chat normal conserva su modelo/proveedor actual.
...
export default async function handler(req, res) {
    ...
    if (autonomous) {
        ...
        if (now - state.lastAutonomousAt < minIntervalMs) {
            // No es un error: le decimos al cliente que espere, sin llamar al modelo.
    ...
    for (const model of modelsToTry) {
        try {
            ...
            if (result?.limited) {
                wasRateLimited = true;
                lastErr = `OpenRouter limitado temporalmente; reintentar en ${Math.ceil((result.retryAfterMs || 60000) / 1000)} s.`;
                continue;
            }

            const { response, data } = result || {};
            if (!response?.ok) {
                lastErr = data?.error?.message || `HTTP ${response?.status || 502}`;
                continue;
            }
    ...
    return res.status(wasRateLimited ? 429 : 502).json({
        error: wasRateLimited
            ? "OpenRouter está temporalmente limitado para el Sandbox."
            : "No se pudo obtener una decisión del agente.",
        detail: lastErr,
    });
}
