...
function buildMessages(role, question, previous, round, node, shared, isFinal = false, attachments = [], operationContext = {}, missionSettings = {}) {
  ...
  const intelligenceContext = missionSettings?.intelligenceContext && typeof missionSettings.intelligenceContext === 'object' ? `\n\nINTELLIGENCE CORE CONTEXT (STRUCTURED, NON-PRIVATE):\n${clean(JSON.stringify(missionSettings.intelligenceContext), 9000)}` : '';
  ...
  const system = isFinal
    ? `You are the final responder for CUT-REAL AI SUPER. Use the internal contributions to answer the original user directly and completely in the user's language. Do not mention the pipeline, models, providers, prompts, internal roles, collaboration graph, or hidden reasoning. Do not output headings such as Conclusions & Findings unless the user explicitly requests a report. Return only the final answer that should be shown to the user. Render Markdown naturally with **bold**, *italics*, headings, lists, tables, and fenced mermaid blocks when a visual scheme is useful. Never claim that a file, Sandbox or Workspace action was executed unless the interface confirms it. ${SUPER_TOOL_BRIEF}${missionContext}${intelligenceContext}${operationText} This is final synthesis round ${round}.`
    : `You are ${roleText} in CUT-REAL AI SUPER, a multi-model collaborative intelligence pipeline. You are not the final responder. Analyze, criticize, correct, improve, and provide evidence for the next model. Do not reveal private chain-of-thought or hidden reasoning; return only conclusions, findings, corrections, uncertainties, and an improved answer. ${SUPER_TOOL_BRIEF}${missionContext}${intelligenceContext}${operationText} This is feedback round ${round}. Your provider/model identity is ${node.provider || 'configured provider'} / ${node.model || 'configured model'}.`;
  const instruction = isFinal
    ? `ORIGINAL USER REQUEST:\n${String(question).slice(0, 5000)}${attachmentBlock}${missionContext}${intelligenceContext}${operationText}${prior}${sharedText}\n\nWrite the final direct answer now. Do not describe your analysis process or the other models. If the request is a simple greeting or question, answer it naturally and concisely.`
    : `ORIGINAL USER REQUEST:\n${String(question).slice(0, 5000)}${attachmentBlock}${missionContext}${intelligenceContext}${prior}${sharedText}\n\nProduce a useful, self-contained contribution for the next stage. Avoid copying without adding verifiable improvement.`;
...
async function collaborate(body) {
  ...
  return {
    ok: Boolean(finalResult), error: failureMessage, failureDetails, mode, rounds, final, steps, events, judge: judgeReport, knowledgeGraph, mission: body.missionContext || null, intelligence: body.intelligenceContext || null, consensus: agreement.label, consensusScore: agreement.score, disagreements: [...agreement.disagreements, ...(judgeReport.disagreements || [])],
