...
(function () {
  ...
  function editorMode(file) { return file === 'index.html' ? 'htmlmixed' : file === 'style.css' ? 'css' : 'javascript'; }
  ...
  function run() {
    ...
    frame.srcdoc = `${project.files['index.html'] || ''}<style>${project.files['style.css'] || ''}</style><script>${project.files['script.js'] || ''}<\/script>`;
  ...
  function applyChanges(project, changes) {
    ...
    for (const change of changes) {
      ...
      if (!file || !Object.prototype.hasOwnProperty.call(project.files, file)) continue;
  ...
  async function send() {
    ...
    const system = `Sos el agente exclusivo del proyecto ${project.name}. Categoría: ${project.category}. No mezcles Sandbox, SUPER ni Chat Normal. Tenés acceso directo a los archivos actuales y debés modificarlos vos mismo cuando el pedido implique código. Nunca le pidas al usuario que copie o pegue el código actual. Respondé ÚNICAMENTE JSON válido, sin Markdown, con esta forma: {"message":"explicación breve en español","changes":[{"file":"index.html|style.css|script.js","operation":"set|replace|append","content":"contenido completo para set/append","oldText":"texto exacto para replace","newText":"reemplazo"}]}. Usá replace para cambios pequeños y set solo cuando sea necesario. Si el usuario pide código, aplicá los cambios directamente. No inventes archivos fuera de los tres disponibles. Revisá siempre el código actual antes de modificarlo. Archivos actuales: ${JSON.stringify(project.files)}`;
    try {
  ...
  function bind() { $('cr-project-close')?.addEventListener('click', close); $('cutreal-project-create')?.addEventListener('click', create); $('cr-project-run')?.addEventListener('click', run); $('cr-project-send')?.addEventListener('click', send); $('cr-project-delete-current')?.addEventListener('click', () => remove()); $('cr-project-share')?.addEventListener('click', shareProject); $('cr-project-import')?.addEventListener('click', importProject); $('cr-project-save-secret')?.addEventListener('click', saveSecret); $('cr-project-reveal-secret')?.addEventListener('click', revealSecret); $('cr-project-editor-expand')?.addEventListener('click', () => toggleProjectPanel('cr-project-editor-shell', 'cr-project-editor-expand')); $('cr-project-preview-expand')?.addEventListener('click', () => toggleProjectPanel('cr-project-preview-shell', 'cr-project-preview-expand')); $('cr-project-model')?.addEventListener('change', event => { const project = active(); if (project) { project.model = event.target.value; save(); renderList(); } }); $('cr-project-input')?.addEventListener('keydown', e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }); }
