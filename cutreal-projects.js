...
(function () {
  ...
  const defaults = () => ({ id: `project-${crypto.randomUUID?.() || Date.now()}`, name: 'Nuevo proyecto', category: 'programacion', model: 'pro', messages: [], files: defaultFiles(), activeFile: 'index.html', documentTitle: 'Nuevo informe', documentHtml: '<h1>Nuevo informe</h1><p>Escribí o pedile a la IA que prepare el contenido.</p>', versions: [] });
  ...
  function normaliseProject(project) {
    const base = defaults();
    return { ...base, ...project, files: { ...defaultFiles(), ...(project?.files || {}) }, messages: Array.isArray(project?.messages) ? project.messages : [], versions: Array.isArray(project?.versions) ? project.versions : [], documentTitle: project?.documentTitle || base.documentTitle, documentHtml: project?.documentHtml || base.documentHtml };
  }
  function load() {
    try {
