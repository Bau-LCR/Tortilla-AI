...
(function () {
  ...
  const defaults = () => ({ id: `project-${crypto.randomUUID?.() || Date.now()}`, name: 'Nuevo proyecto', category: 'programacion', mode: 'programacion', model: 'pro', messages: [], files: defaultFiles(), activeFile: 'index.html', documentTitle: 'Nuevo informe', documentHtml: '<h1>Nuevo informe</h1><p>Escribí o pedile a la IA que prepare el contenido.</p>', versions: [] });
  ...
  function normaliseProject(project) {
    ...
    return { ...base, ...project, category, mode, files, messages: Array.isArray(project?.messages) ? project.messages : [], versions: Array.isArray(project?.versions) ? project.versions : [], documentTitle: project?.documentTitle || base.documentTitle, documentHtml: project?.documentHtml || base.documentHtml };
  }
  function load() {
    try {
  ...
  function snapshot(project) { return { files: clone(project.files), activeFile: project.activeFile, documentTitle: project.documentTitle, documentHtml: project.documentHtml, savedAt: Date.now() }; }
  ...
  function renderDocument() {
    ...
    const documentMode = isDocumentProject(project);
  ...
  async function send() {
    ...
    try {
      ...
      let changes = Array.isArray(result.payload.changes) ? result.payload.changes : []; let documentApplied = applyDocument(project, result.payload.document);
      ...
      if (!changes.length && implementation) {
        ...
        result = await askProjectAgent(project, repairSystem, repairMessages); changes = Array.isArray(result.payload.changes) ? result.payload.changes : extractCodeChanges(result.rawReply, project.files); documentApplied = applyDocument(project, result.payload.document) || documentApplied;
      }
      if (thinking.isConnected) thinking.remove();
      const applied = applyChanges(project, changes);
      if (applied.count || documentApplied) { recordVersion(project, `IA: ${text.slice(0, 100)}`); project.messages.push({ role: 'assistant', content: `${result.payload.message || (documentApplied ? 'Documento actualizado automáticamente.' : 'Cambios aplicados automáticamente.')}\n\n${documentApplied ? 'Documento actualizado en tiempo real.' : `Archivos actualizados: ${applied.files.join(', ')}.`}` }); save(); render(); if (!documentMode) run(); setStatus(documentApplied ? 'Documento actualizado · guardado' : `IA aplicó cambios en ${applied.files.join(', ')} · guardado`); }
  ...
  function restoreVersion(versionIndex = 0) { const project = active(); const version = project?.versions?.[versionIndex]; if (!project || !version) return false; recordVersion(project, 'Antes de restaurar una versión'); project.files = clone(version.files); project.activeFile = version.activeFile || 'index.html'; project.documentTitle = version.documentTitle || project.documentTitle; project.documentHtml = version.documentHtml || project.documentHtml; save(); render(); run(); return true; }
