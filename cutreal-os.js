...
(function () {
    ...
    root.innerHTML = `<button id="cutreal-os-launch" class="cro-launch" type="button" title="Abrir Command Center">◈ CUT-REAL OS</button>
      <div id="cro-drawer" class="cro-drawer" hidden>
       ...
       <nav class="cro-tabs" aria-label="Módulos CUT-REAL OS">
        <button data-cro-tab="dashboard" class="active">Dashboard</button><button data-cro-tab="capabilities">Capacidades</button><button data-cro-tab="desktop">Desktop</button><button data-cro-tab="projects">Projects</button><button data-cro-tab="memory">Memory</button><button data-cro-tab="skills">Skills</button><button data-cro-tab="files">Vault</button><button data-cro-tab="workflows">Workflows</button><button data-cro-tab="permissions">Permisos</button>
       </nav>
  ...
  function render(tab='dashboard') {
    ...
    const views = {
      ...
      capabilities: `<div class="cro-grid">${(window.CutRealCapabilities?.listCapabilities?.() || []).map(item => card(esc(item.label), `<p class="cro-muted">Estado: <b>${esc(item.status)}</b></p><div class="cro-capability-list">${(item.features || []).map(feature => `<span class="cro-list-row">${esc(feature)}</span>`).join('')}</div>`)).join('')}</div>`,
