...
function styleCanvas(canvas) {
    ...
    // position:fixed cubriendo todo el viewport: así se ve en pantalla
    // completa sin depender del CSS del contenedor (#doom-overlay).
    // object-fit:contain mantiene la relación de aspecto original de DOOM
    // (agrega franjas negras arriba/abajo en monitores muy panorámicos).
    ...
    canvas.style.imageRendering = 'pixelated'; // look retro a pixeles — cambiar a 'auto' si preferís suavizado
    canvas.style.zIndex         = '1';
    canvas.tabIndex = 0; // para que pueda recibir foco

    if (!canvas.dataset.doomClickBound) {
        canvas.addEventListener('click', () => canvas.focus());
        canvas.dataset.doomClickBound = '1';
    }

    ensureOverlayChildrenOnTop();
}

// Garantiza que el botón "Cerrar" y la barra de controles queden siempre
// por encima del canvas (que ahora es fixed a pantalla completa).
function ensureOverlayChildrenOnTop() {
    const closeBtn = document.getElementById('doom-close-btn');
    if (closeBtn) {
        closeBtn.style.position = 'fixed';
        closeBtn.style.top      = '14px';
        closeBtn.style.right    = '14px';
        closeBtn.style.zIndex   = '99300';
    }
    const controls = document.getElementById('doom-controls');
    if (controls) {
        controls.style.position   = 'fixed';
        controls.style.left       = '0';
        controls.style.right      = '0';
        controls.style.bottom     = '0';
        controls.style.zIndex     = '99250';
        controls.style.textAlign  = 'center';
        controls.style.background = 'rgba(0,0,0,0.55)';
        controls.style.padding    = '6px 0';
    }
}
...
async function startDoom() {
    ...
    try {
        ...
        doomInstance = new DOOM({
            ...
            // Sin keyboardTarget: usa el comportamiento por defecto de la
            // librería (escucha en document/window), igual que su demo
            // oficial. Fijarlo al canvas requería que el foco del navegador
            // quedara siempre sobre él, lo cual fallaba y hacía que ninguna
            // tecla (incluido ENTER para arrancar) llegara al motor — por
            // eso el juego caía en su "demo automática" sin que dispararas.
